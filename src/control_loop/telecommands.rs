...
impl FlipFlopState {
    pub(super) fn from_u8(uint: u8) -> Result<FlipFlopState, ParseError> {
        if uint >= size_of::<FlipFlopState>() as u8 {
            Err(ParseError)
...
impl Telecommand {
    pub fn parse(data: &[u8]) -> Result<Self, ParseError> {
        if data.len() < 5 { return Err(ParseError); }
        if data[0] != POWER_CMD_SUBSYSTEM_ID { return Err(ParseError); }
